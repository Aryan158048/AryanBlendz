...
function DatePicker({ slot, onClose }: SlotPickerProps) {
  ...
  return (
    <div className="mt-2 p-3 sm:p-4 rounded-xl bg-charcoal-900 border border-white/15 space-y-2 animate-fade-in">
      ...
      {dates.map((date) => (
        ...
        >
          ...
          <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white/25 group-hover:text-[#CC0033] transition-colors flex-shrink-0" />
        </button>
      ))}
      <button
        onClick={onClose}
...
export function AvailableSlotsView({ slots, settings }: Props) {
  ...
  if (totalSlots === 0) {
    return (
      <div className="text-center py-8 sm:py-12 space-y-3">
        <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-white/6 flex items-center justify-center mx-auto">
          <Calendar className="w-5 h-5 sm:w-6 sm:h-6 text-white/25" />
        </div>
        <div>
          <p className="text-white/60 text-xs sm:text-sm font-medium">No mutual free times found</p>
  ...
  return (
    <div className="space-y-4">
      ...
      {SCHEDULE_DAYS.map((day) => {
        ...
        return (
          <div key={day} className="rounded-xl border border-white/8 overflow-hidden">
            {/* Day header */}
            <div className="px-3 sm:px-3.5 py-2 sm:py-2.5 bg-white/4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="text-white/75 text-xs sm:text-sm font-semibold">{DAY_LABELS[day]}</span>
                <span className="text-[9px] sm:text-[10px] bg-green-500/15 text-green-400 px-1.5 py-0.5 rounded-full">
                  {daySlots.length} open
                </span>
              </div>
              <Clock className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-white/20" />
            </div>

            {/* Slot pills */}
            <div className="px-3 sm:px-3.5 py-3 flex flex-wrap gap-2">
              {daySlots.map((slot) => {
                ...
                return (
                  <div key={key} className="w-full sm:w-auto">
                    <button
                      ...
                      className={cn(
                        'flex items-center gap-2 px-3 py-2 rounded-xl border text-xs sm:text-sm font-medium transition-all w-full sm:w-auto',
                        isOpen
                    ...
                    >
                      ...
                      <ChevronRight className={cn(
                        'w-3 h-3 sm:w-3.5 sm:h-3.5 transition-transform',
      ...
      {/* Footer tip */}
      <p className="text-white/25 text-[10px] sm:text-xs text-center pt-1">
