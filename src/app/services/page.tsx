...
const categoryIncludes: Record<string, string[]> = {
  haircut:  ['Consultation & style recommendation', 'Precision cut with finish', 'Styled to perfection'],
  beard:    ['Beard consultation & shaping plan', 'Precision trim & line definition', 'Hot towel finish & beard oil'],
  lineup:   ['Edge-up along hairline, temples & nape', 'Straight razor precision', 'Clean finish with styling product'],
  styling:  ['Edge-up along hairline, temples & nape', 'Straight razor precision', 'Clean finish with styling product'],
  combo:    ['Full precision haircut', 'Beard trim, shaping & line definition', 'Hot towel service & premium product finish'],
  kids:     ['Kid-friendly, gentle approach', 'Classic or modern style', 'Styled & finished with care'],
  premium:  ['Haircut + beard with priority booking', 'Premium hair & beard product treatment', 'Scalp massage, hot towel & styling session'],
  other:    ['Expert service', 'Premium products', 'Professional finish'],
}

const FAQS = [
  {
    question: 'Do I need to book in advance?',
    answer: 'We highly recommend booking ahead to secure your preferred time. Walk-ins are welcome based on availability, but slots fill up fast — especially on weekends.',
  },
  {
  ...
  },
  {
    ...
    answer: 'Absolutely. During booking you can select your preferred barber or choose "Any Available" to get the next open slot.',
  },
  {
...
export default async function ServicesPage() {
  const services = await getActiveServices().catch(() => [] as any[])
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {services.map((service: any) => {
              const cat = (service.category ?? 'other').toLowerCase()
              const icon = categoryIcons[cat] ?? '💈'
              const includes = categoryIncludes[cat] ?? categoryIncludes.other
              return (
                <div
                  key={service.id}
